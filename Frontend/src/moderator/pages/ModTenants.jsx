...
import { FiUser, FiX, FiCheckCircle, FiSlash, FiUserCheck, FiTrash2, FiAlertTriangle } from "react-icons/fi";
  ...
  const columns = [
    ...
    { key: "actions", label: "Actions", render: (t) => (
      <div style={cs.actions}>
        {has("tenants:update") && <button style={{ ...cs.btn, background: t.blocked ? "#eafaf1" : "#fdf0f0", color: t.blocked ? "#27ae60" : "#e74c3c", border: t.blocked ? "1px solid #a9dfbf" : "1px solid #f5c6cb" }} onClick={() => toggle(t._id)}>{t.blocked ? <><FiUserCheck size={12} style={{ marginRight: 4 }} />Unblock</> : <><FiSlash size={12} style={{ marginRight: 4 }} />Block</>}</button>}
        {has("tenants:delete") && <button style={{ ...cs.btn, background: "#fdf0f0", color: "#e74c3c", border: "1px solid #f5c6cb" }} onClick={() => setDeleteTarget(t)}><FiTrash2 size={12} style={{ marginRight: 4 }} />Delete</button>}
