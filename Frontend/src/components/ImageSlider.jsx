...
export default function ImageSlider({ images, image, height = "190px", noImgSize = "3rem", onImageClick, labels, imageLabels }) {
  ...
  return (
    <div style={{ position: "relative", height, overflow: "hidden", flexShrink: 0 }}>
      <img
        src={src(all[idx])}
        alt="flat"
        onClick={onImageClick ? (e) => { e.preventDefault(); e.stopPropagation(); onImageClick(all.map(src), idx, imageLabels || labels); } : undefined}
        style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", cursor: onImageClick ? "zoom-in" : "default" }}
      />
      {(labels?.[idx] || imageLabels?.[idx]) && (
        <span style={{ position: "absolute", bottom: "8px", left: "8px", background: "linear-gradient(135deg, #1abc9c, #16a085)", color: "#fff", fontSize: "0.78rem", fontWeight: "700", padding: "4px 12px", borderRadius: "20px", pointerEvents: "none", letterSpacing: "0.3px" }}>
          {labels?.[idx] || imageLabels?.[idx]}
        </span>
      )}
      {all.length > 1 && (
