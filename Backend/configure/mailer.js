const LOGO_URL = "https://res.cloudinary.com/ds5ef65br/image/upload/v1778390455/flatkart_logo.png";
...
export const sendOTPEmail = async (to, otp, subject = "Reset Your Password", heading = "Email Verification", bodyText = "Use the OTP below to verify your email:") => {
  const res = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      subject,
      htmlContent: `
      <div style="font-family:sans-serif;max-width:480px;margin:auto;border:1px solid #eee;border-radius:12px;overflow:hidden">
        <div style="background:linear-gradient(135deg,#1a252f,#2c3e50);padding:24px 32px;text-align:center">
          <img src="${LOGO_URL}" alt="Flatkart" style="height:56px;width:auto;display:block;margin:0 auto" />
        </div>
        <div style="padding:32px">
          <h2 style="color:#2c3e50;margin:0 0 12px">${heading}</h2>
          <p style="color:#555;margin:0 0 8px">${bodyText}</p>
          ...
          <p style="color:#888;font-size:0.9rem">If you did not request this, please contact our support team at <a href="mailto:flatkart.support@gmail.com" style="color:#1abc9c">flatkart.support@gmail.com</a>.</p>
        </div>
        <div style="background:#f0f2f5;padding:14px 32px;text-align:center;border-top:1px solid #eee">
          <p style="color:#aaa;font-size:0.78rem;margin:0">&copy; ${new Date().getFullYear()} Flatkart. All rights reserved.</p>
        </div>
      </div>`,
    }),
  });
